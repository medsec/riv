...
static inline void clhash_short_message(const clhash_ctx_t* ctx, 
                                         __m128i* h, 
                                         const uint64_t hlen, 
                                         ...
                                         uint8_t target[CLHASH_HASHLEN])
{
    ...
    short_tail(ctx->k_pprime[0], nh_sum1, num_bytes, target);
    short_tail(ctx->k_pprime[1], nh_sum2, num_bytes, target+8);
}
...
static inline void clhash_long_message(const clhash_ctx_t* ctx, 
                                       __m128i* h, 
                                       const uint64_t hlen, 
                                       const uint8_t domain, 
                                       __m128i* m, 
                                       ...
                                       uint8_t* target)
{
    ...
    __m128i* nh_key = (__m128i*)(ctx->nh_key);
    __m128i* poly_key = (__m128i*)(ctx->k);
    ...
    while (m_pos + 4 * CL_NH_BLOCKLEN <= mlen) {
        ...
        poly_sum1 = gf128_multiply_four((__m128i*)ctx->k, poly_terms1);
        poly_sum2 = gf128_multiply_four(
            (__m128i*)ctx->k+CL_NUM_PRECOMPUTED_POWERS_OF_K, 
            poly_terms2
        );
    ...
    long_tail(ctx->k_prime,   ctx->k_pprime[0], 
        vxor(poly_sum1, poly_term1), num_bytes, target);
    long_tail(ctx->k_prime+2, ctx->k_pprime[1], 
            ...
            uint8_t target[CLHASH_HASHLEN])
{
    ...
    if (num_bytes <= CL_NH_BLOCKLEN) {
        clhash_short_message(ctx, h, num_header_bytes, domain, 
            m, num_message_bytes, num_bytes, target);
    } else {
        clhash_long_message(ctx, h, num_header_bytes, domain, 
            m, num_message_bytes, num_bytes, target);
    }
}
...
void clhash_keysetup(clhash_ctx_t* ctx, const uint8_t key[CLHASH_KEYLEN])
{
    ...
    memcpy(ctx->k, key + CL_NH_KEYLEN, BLOCKLEN);
    memcpy(ctx->k+CL_NUM_BYTES_BETWEEN_K, key + CL_NH_KEYLEN+BLOCKLEN, BLOCKLEN);
    
    // Each k is a 126-bit integer, and k[15] the highest byte.
    ctx->k[BLOCKLEN-1] &= MASK_126; 
    ctx->k[BLOCKLEN-1+CL_NUM_BYTES_BETWEEN_K] &= MASK_126;
    
    memcpy((uint8_t*)&(ctx->k_prime), key + CL_NH_KEYLEN + CL_K_LEN, 
        CL_K_PRIME_LEN);
    memcpy((uint8_t*)&(ctx->k_pprime), 
        key + CL_NH_KEYLEN + CL_K_LEN + CL_K_PRIME_LEN, 
        CL_K_PPRIME_LEN);

    precompute_poly_keys(ctx->k);
    precompute_poly_keys(ctx->k+CL_NUM_BYTES_BETWEEN_K);
}
