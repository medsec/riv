...
static void clhash_short_message(const clhash_ctx_t* ctx, 
                                 const uint8_t* h, 
                                 const uint64_t hlen, 
                                 const uint8_t domain, 
                                 const uint8_t* m, 
                                 ...
                                 uint8_t target[CLHASH_HASHLEN])
{
    ...
    clnh_partial_block(ctx->nh_key, chunk, chunklen, nh_sum1, nh_sum2);
    
    block size_hash1;
    block size_hash2;
    hash_size(ctx->k_pprime[0], num_bytes, size_hash1);
    hash_size(ctx->k_pprime[1], num_bytes, size_hash2);
...
static void clhash_long_message(const clhash_ctx_t* ctx, 
                                const uint8_t* h, 
                                const uint64_t hlen, 
                                const uint8_t domain, 
                                const uint8_t* m, 
                                ...
                                uint8_t* target)
{
    ...
    while (h_pos + CL_NH_BLOCKLEN <= hlen) {
        clnh_full_block(ctx->nh_key, h+h_pos, poly_term1, poly_term2);
        poly_step(ctx->k,   poly_sum1, poly_term1, poly_sum1);
        poly_step(ctx->k+2, poly_sum2, poly_term2, poly_sum2);
    ...
    if (chunklen == CL_NH_BLOCKLEN) {
        clnh_full_block(ctx->nh_key, chunk, poly_term1, poly_term2);
    } else {
        clnh_partial_block(ctx->nh_key, chunk, chunklen, 
    ...
    if (!processed_length) {
        ...
        while (m_pos + CL_NH_BLOCKLEN <= mlen) {
            clnh_full_block(ctx->nh_key, m+m_pos, poly_term1, poly_term2);
            poly_step(ctx->k,   poly_sum1, poly_term1, poly_sum1);
            poly_step(ctx->k+2, poly_sum2, poly_term2, poly_sum2);
        ...
        if (chunklen == CL_NH_BLOCKLEN) {
            ...
            if (!processed_length) {
                ...
                clnh_partial_block(ctx->nh_key, chunk, BLOCKLEN, 
                    poly_term1, poly_term2);
                m_pos = mlen;
            }
        } else {
    ...
    hash_asu(ctx->k_prime,   poly_sum1, inner_product_hash1);
    hash_asu(ctx->k_prime+2, poly_sum2, inner_product_hash2);
            ...
            unsigned char target[CLHASH_HASHLEN])
{
    ...
    clhash_long_message(ctx, 
        h, num_header_bytes, domain, m, num_message_bytes, num_bytes, target);
}
