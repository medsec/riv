/**
// RIV[CLHASH-128, XOR-CTR[AES-128]] x86-64-only code.
                    ...
                    uint8_t* ciphertext)
{
    ...
    counter_mode(ctx, iv_encrypted, m, plaintext_length, c);
}
                    ...
                    const uint8_t* ciphertext)
{
    ...
    __m128i* c = (__m128i*)ciphertext;
    __m128i iv_encrypted = aes_encrypt(iv, ctx->expanced_enc_key);
    counter_mode(ctx, iv_encrypted, c, ciphertext_length, m);
}
...
{
    ALIGN(16) uint8_t iv[BLOCKLEN];
    ALIGN(16) uint8_t s[BLOCKLEN];
    memset(iv, 0x00, BLOCKLEN);
    memset(s, 0x00, BLOCKLEN);

    clhash(&(ctx->prf_context), header, header_length, DOMAIN_0, 
        plaintext, plaintext_length, iv);

    __m128i iv_ = aes_encrypt(load(iv), ctx->expanced_enc_key);
    ...
    __m128i s_ = aes_encrypt(load(s), ctx->expanced_enc_key);
    storeu(tag, vxor(s_, iv_));
}
...
{
    ALIGN(16) uint8_t iv[BLOCKLEN];
    ALIGN(16) uint8_t iv_prime[BLOCKLEN];

    clhash(&(ctx->prf_context), 
        header, header_length, DOMAIN_1, ciphertext, ciphertext_length, iv);
    __m128i iv_ = vxor(aes_encrypt(load(iv), ctx->expanced_enc_key), loadu(tag));

    decrypt(ctx, iv_, plaintext, ciphertext_length, ciphertext);
    
    clhash(&(ctx->prf_context), 
        header, header_length, DOMAIN_0, plaintext, ciphertext_length, iv_prime);
    __m128i iv_prime_ = aes_encrypt(load(iv_prime), ctx->expanced_enc_key);
    return _mm_testc_si128(iv_, iv_prime_) - 1;
}
